function varargout = vectorGradient(f,varargin)
...
for k = 1:ndim
   ...
   if n > 2
      h = h(3:n) - h(1:n-2);
      h1 = h(:,ones(p,1), ones(vecdim,1));
      h2 = permute(h1, [3 1 2]);
      g(:,2:n-1,:) = (f(:,3:n,:)-f(:,1:n-2,:))./h2;
   end

   varargout{k} = ipermute(g,[0 k:ndim 1:k-1]+1);
   varargout{k} = permute(varargout{k}, [2:ndim+1 1]);

   % Set up for next pass through the loop
   f = permute(f,[1 perm+1]);
end 
